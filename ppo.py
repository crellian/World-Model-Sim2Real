...
class PolicyGraph():
    ...
    def __init__(self, input_states, taken_actions, action_space, scope_name,
        ...
        with tf.variable_scope(scope_name):
            ...
            self.action_logstd = tf.Variable(np.full((num_actions), np.log(initial_std), dtype=np.float32),
                                             name="action_logstd")
            ...
            if vf_hidden_sizes is None:
                ...
                self.vf = build_mlp(input_states, hidden_sizes=vf_hidden_sizes, activation=tf.nn.relu,
                                    output_activation=tf.nn.relu)
            self.value = tf.squeeze(tf.layers.dense(self.vf, 1, activation=None, name="value"), axis=-1)

            # Create graph for sampling actions
            self.action_normal = tfp.distributions.Normal(self.action_mean, tf.exp(self.action_logstd),
                                                          validate_args=True)
...
class PPO():
    ...
    def __init__(self, input_shape, action_space,
        ...
        self.taken_actions = tf.placeholder(shape=(None, num_actions), dtype=tf.float32,
        ...
        self.policy = PolicyGraph(self.input_states, self.taken_actions, action_space, "policy",
                                  initial_std=initial_std)
        self.policy_old = PolicyGraph(self.input_states, self.taken_actions, action_space, "policy_old",
                                      initial_std=initial_std)
        ...
        self.policy_loss = tf.reduce_mean(
            tf.minimum(self.prob_ratio * adv, tf.clip_by_value(self.prob_ratio, 1.0 - epsilon, 1.0 + epsilon) * adv))
        ...
        # Minimize loss
        self.learning_rate = tf.train.exponential_decay(learning_rate, self.episode_counter.var, 1, lr_decay,
                                                        staircase=True)
        ...
        for i in range(num_actions):
            metrics["train_actor/action_{}/taken_actions".format(i)] = tf.metrics.mean(
                tf.reduce_mean(self.taken_actions[:, i]))
            metrics["train_actor/action_{}/mean".format(i)] = tf.metrics.mean(
                tf.reduce_mean(self.policy.action_mean[:, i]))
            metrics["train_actor/action_{}/std".format(i)] = tf.metrics.mean(
        ...
        for i in range(num_actions):
            summaries.append(
                tf.summary.histogram("train_actor_step/action_{}/taken_actions".format(i), self.taken_actions[:, i]))
            summaries.append(
                tf.summary.histogram("train_actor_step/action_{}/mean".format(i), self.policy.action_mean[:, i]))
            summaries.append(
                tf.summary.histogram("train_actor_step/action_{}/std".format(i), tf.exp(self.policy.action_logstd[i])))
        ...
        for i in range(num_actions):
            summaries.append(
                tf.summary.scalar("predict_actor/action_{}/sampled_action".format(i), self.policy.sampled_action[0, i]))
            summaries.append(tf.summary.scalar("predict_actor/action_{}/mean".format(i), self.policy.action_mean[0, i]))
            summaries.append(
                tf.summary.scalar("predict_actor/action_{}/std".format(i), tf.exp(self.policy.action_logstd[i])))
    ...
    def train(self, input_states, taken_actions, returns, advantage):
        _, _, summaries, step_idx = \
            self.sess.run(
                [self.train_step, self.update_metrics_op, self.stepwise_summaries, self.train_step_counter.var],
                feed_dict={
    ...
    def predict(self, input_states, greedy=False, write_to_summary=False):
        ...
        sampled_action, value, summaries, step_idx = \
            self.sess.run(
                [action, self.policy.value, self.stepwise_prediction_summaries, self.predict_step_counter.var],
    ...
    def reset_episode_idx(self):
        self.sess.run(self.episode_counter.reset_op)

    def get_train_step_idx(self):
        return self.sess.run(self.train_step_counter.var)

    def reset_train_step_idx(self):
        self.sess.run(self.train_step_counter.reset_op)

    def get_predict_step_idx(self):
        return self.sess.run(self.predict_step_counter.var)

    def reset_predict_step_idx(self):
        self.sess.run(self.predict_step_counter.reset_op)
    ...
    def write_dict_to_summary(self, summary_name, params, step):
        summary_op = tf.summary.text(summary_name,
                                     tf.stack([tf.convert_to_tensor([k, str(v)]) for k, v in params.items()]))
